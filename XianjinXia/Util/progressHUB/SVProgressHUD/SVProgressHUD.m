...
- (void)showLoading{
    ...
    CustomLoadingView *loadingView = [CustomLoadingView sharedLoadingView:NO];
    [loadingView bringSubviewToFront:loadingView.loadLabel];
}
+(void)showLoading
{
    [self dismiss];
...
+ (void)dismissWithDelay:(NSTimeInterval)delay{
    ...
    [[self sharedView] dismissWithDelay:delay completion:nil];
...
+ (void)dismiss{
    [self dismissWithDelay:.1f];
}

+ (void)dismissLoadingView
{
    CustomLoadingView *loadingView = [CustomLoadingView sharedLoadingView:[SVProgressHUD sharedView].defaultMaskType == SVProgressHUDMaskTypeNone];

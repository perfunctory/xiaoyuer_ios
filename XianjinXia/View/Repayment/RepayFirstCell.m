...
@property (nonatomic, strong) UILabel       * lblExtraMoney;
@property (nonatomic, strong) UIImageView   * imvJump;

@end

@implementation RepayFirstCell

+ (RepayFirstCell *)repayFirstCellWithTableView:(UITableView *)tableView {
    static NSString * ID = @"KDRepayFirstCell";
    RepayFirstCell * cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if ( !cell ) {
        cell = [[RepayFirstCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
...
- (void)setupUI {
    ...
    [self.contentView addSubview:self.lblExtraMoney];
...
- (void)updateConstraints {
    ...
    [self.lblExtraMoney mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(weakSelf.vVer.mas_right).offset(15*WIDTHRADIUS);
        make.top.equalTo(weakSelf.lblLoanPro.mas_bottom).offset(5*WIDTHRADIUS);
    }];
...
- (void)configCellWithModel:(RepayAmountModel *)model withIndexPath:(NSIndexPath *)indexPath {
    ...
    NSMutableAttributedString * extraFee = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"逾期费用: %@元",model.late_fee]];
    [extraFee addAttribute:NSForegroundColorAttributeName value:Color_Overdue range:NSMakeRange(5, model.late_fee.length + 2)];
    self.lblExtraMoney.attributedText = extraFee;
...
- (UILabel *)lblExtraMoney {
    if (!_lblExtraMoney) {
        _lblExtraMoney = [self getLabelWithFontSize:Repayment_SubTitle textColor:Color_SubTitle withText:@"逾期费用:---"];
    }
    return _lblExtraMoney;
}
